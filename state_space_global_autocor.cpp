...
//The only difference between this file and "stateSpaceGlobalAutocor2.cpp"
...
Type objective_function<Type>::operator() (){
  ...
  //correlations between random spatioLevel effects, random slopes of days and random slopes of squared days 
  //PARAMETER_VECTOR(rhoRan);
  ...
  //contribution of random intercepts and slopes of days and squaerd days to likelihood
  // when they are correlated
  // if (CppAD::Variable(mu_spatioLevel(0)) &&  CppAD::Variable(mu_spatioLevel_days(0)) && CppAD::Variable(mu_spatioLevel_sqdays(0))){
  //     vector<Type>  ranEff(3);
  //     vector<Type>  sdVecRan(3);
  //     matrix<Type>  CorRan(3,3); 
  //        
  //        sdVecRan(0) =  sd_spatioLevel;
  //        sdVecRan(1) =  sd_randm_slop_days;
  //        sdVecRan(2) =  sd_randm_slop_sqdays;
  // 
  //        CorRan(0,0) = 1; CorRan(0,1) = rhoRan(0); CorRan(0,2) = rhoRan(1);
  //        CorRan(1,0) = rhoRan(0); CorRan(1,1) = 1; CorRan(1,2) = rhoRan(2);
  //        CorRan(2,0) = rhoRan(1); CorRan(2,1) = rhoRan(2); CorRan(2,2) = 1;
  //  
  //    for (int j=0; j<nspatioLevel; ++j){
  //         ranEff(0)=mu_spatioLevel(j);
  //         ranEff(1)=mu_spatioLevel_days(j);
  //         ranEff(3)=mu_spatioLevel_sqdays(j);
  //         nll += VECSCALE(MVNORM(CorRan),sdVecRan)(ranEff); }
  //      }else{
  ...
  for (int i=0; i<nObs; i++){
    ...
    //nll -= dnorm(lognewcasessmooth(i), state(i), sd_error_measure, true);
